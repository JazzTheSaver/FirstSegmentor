#include "SemiEmbLSTMSegmentor.h"
...
int Classifier::createAlphabet(const vector<Instance>& vecInsts) {
	...
	m_driver._modelparams.r_charsAlpha.clear();
	m_driver._modelparams.bicharsAlpha.clear();
	m_driver._modelparams.r_bicharsAlpha.clear();
	for (numInstance = 0; numInstance < vecInsts.size(); numInstance++) {
		...
		m_driver._modelparams.r_charsAlpha.initial(m_char_stats, m_options.charCutOff);
		...
		m_driver._modelparams.bicharsAlpha.initial(m_bichar_stats);
		m_driver._modelparams.r_bicharsAlpha.initial(m_bichar_stats, m_options.bicharCutOff);
	...
	cout << "random_Chars num: " << m_driver._modelparams.r_charsAlpha.size() << endl;
	cout << "BiChars num: " << m_driver._modelparams.bicharsAlpha.size() << endl;
	cout << "random_BiChars num: " << m_driver._modelparams.r_bicharsAlpha.size() << endl;
...
int Classifier::addTestAlpha(const vector<Instance>& vecInsts) {
	...
	//	const vector<string> &chars = pInstance->m_words;
	//	int chars_num = chars.size();
	//	for (int i = 0; i < chars_num; i++) {
	//		m_char_stats[chars[i]]++;
	//	}

	//	m_driver._modelparams.charsAlpha.initial(m_char_stats, m_options.wordCutOff);
...
void Classifier::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile) {
	...
	m_driver._modelparams.chars_r.initial(&m_driver._modelparams.charsAlpha, m_options.charEmbSize, !m_options.charEmbFineTune);
	m_driver._modelparams.l_bichars_r.initial(&m_driver._modelparams.bicharsAlpha, m_options.bicharEmbSize, !m_options.bicharEmbFineTune);
	m_driver._modelparams.r_bichars_r.initial(&m_driver._modelparams.bicharsAlpha, m_options.bicharEmbSize, !m_options.bicharEmbFineTune);
